...
public class CafeApp {
    ...
    private static final File CREDENTIALS_FILE = new File("/Users/jaifha_wongkunta/Documents/wuttikan/DII-GEN6-CompoundSEC/user.txt");
    private static final File STOCK_FILE = new File("/Users/jaifha_wongkunta/Documents/wuttikan/DII-GEN6-CompoundSEC/stock.txt");


    public CafeApp() {
        frame = new JFrame("Cafe authentication");
        ...
        ImageIcon originalIcon = new ImageIcon("/Users/jaifha_wongkunta/Documents/wuttikan/CafeWifiSystem/logo/LOGO.png");
    ...
    private void showStock() {
        if (!STOCK_FILE.exists()) {
            JOptionPane.showMessageDialog(frame, "Stock file not found.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        StringBuilder stockContent = new StringBuilder();
        try (Scanner scanner = new Scanner(STOCK_FILE)) {
            while (scanner.hasNextLine()) {
                stockContent.append(scanner.nextLine()).append("\n");
            }
        } catch (IOException ex) {
            ex.printStackTrace();
        }

        JTextArea stockTextArea = new JTextArea(stockContent.toString());
        stockTextArea.setEditable(false);
        JScrollPane scrollPane = new JScrollPane(stockTextArea);
        scrollPane.setPreferredSize(new Dimension(400, 300));

        JFrame stockFrame = new JFrame("Stock List");
        stockFrame.setSize(450, 400);
